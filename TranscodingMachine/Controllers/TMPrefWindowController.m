...
@implementation TMPrefWindowController
...
- (id)initWithController:(TMAppController *)controller {
    if (self = [super initWithController: controller withNibName:@"Preferences"]){
        NSAssert([self window], @"[TMPrefWindowController init] window outlet is not connected in Preferences.nib");
...
- (IBAction) browseDirectory: (id) sender{
	...
	[panel beginSheetModalForWindow:self.window completionHandler:^(NSInteger result) {
		if( result == NSOKButton ){
			if (sender == browseMonitoredButton) {
				[monitoredFolderField setStringValue:[panel.directoryURL path]];
			}else{
				[outputFolderField setStringValue:[panel.directoryURL path]];
			}
